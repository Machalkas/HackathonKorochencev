...
function update() {
  ...
  if (checkp != undefined && checkp.find(item => item.id == active_checkpoint) != undefined && isActive(checkp.find(item => item.id == active_checkpoint)) == false) {
    window.location.reload();
  }
  console.log(active_checkpoint+"|"+checkp[0].id+"|"+checkp[1].id);
}
